...
impl NoGenericArgsGenericLibfunc for Sha256CompressLibfunc {
    ...
    fn specialize_signature(
        ...
        let u32x8_ty = u32_fixed_array_ty(context, 8)?;
        let u32x16_ty = u32_fixed_array_ty(context, 16)?;
        Ok(LibfuncSignature::new_non_branch_ex(
            vec![
                ParamSignature::new(sha256_ty.clone()),
                ParamSignature::new(u32x8_ty.clone()), // Hasher state
                ParamSignature::new(u32x16_ty),        // Current block
            ],
            vec![
                OutputVarInfo::new_builtin(sha256_ty, 0),
                OutputVarInfo {
                    ty: box_ty(context, u32x8_ty)?,
...
/// Returns `[u32; size]` according to the given size.
fn u32_fixed_array_ty(
    context: &dyn SignatureSpecializationContext,
    size: i16,
) -> Result<ConcreteTypeId, SpecializationError> {
    let ty = context.get_concrete_type(Uint32Type::id(), &[])?;
    fixed_size_array_ty(context, ty, size)
}
